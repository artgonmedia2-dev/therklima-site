      ...
      <section className="py-24 md:py-32 bg-[#0f172a]" aria-label="Nos réalisations">
        <div className="container-custom text-center">
          ...
          <p className="text-white/70 text-lg max-w-xl mx-auto">
            Découvrez quelques-uns de nos projets récents — 1 réalisation par métier sur Paris et Île-de-France.
          </p>
        </div>
      </section>
      ...
      <section className="section-padding bg-[#f8fafc]" aria-labelledby="realisations-heading">
        <div className="container-custom">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {REALISATIONS.map((r) => (
              <article key={r.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                {/* Image placeholder */}
                <div
                  className="h-48 relative"
                  style={{ background: `linear-gradient(135deg, ${r.metierColor}20, ${r.metierColor}40)` }}
                  role="img"
                  aria-label={r.alt}
                >
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-5xl font-bold opacity-20" style={{ color: r.metierColor }}>
                      {r.metierName.charAt(0)}
                    </span>
                  </div>
                  <span
